...
/**
    ...
    public function getLoadedProductCollection($query){
            ...
            //$productCollection->addAttributeToFilter('price', array('gt' => 0)); Comment for get products bundle
        ...
        foreach ($_productCollection as $product) {
            ...
            $data['description'] = $product->getDescription();
             if($product->getTypeId() === 'bundle'){
                $data['price'] = $product->getData('price');
                $data['min_price'] = $product->getData('min_price');
                $data['max_price'] = $product->getData('max_price');
            }else {
                $data['price'] = $product->getData('price');
                $data['min_price'] = $product->getData('min_price');
