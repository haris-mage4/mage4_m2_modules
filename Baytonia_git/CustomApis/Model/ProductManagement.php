...
class ProductManagement
{
    ...
    public function __construct(\Magento\Store\Model\StoreManagerInterface $storeInterface, \Magento\Catalog\Model\ProductFactory $_productloader, \Magento\ConfigurableProduct\Model\Product\Type\Configurable
                ...
                $tempArray["attribute_code"] = $optionData["attribute_code"];
                   ...
                   if (isset($optionData["values"])) {
                    foreach ($optionData["values"] as $value) {
                    	/** get product child id  */
                        $id = $value["value_index"];
                        $attributesInfo = [$attrid => $id];
                        $childProduct = $this->_customOptions->getProductByAttributes($attributesInfo ,$prd_data);
                        
                        $tempArray["options"][] = array(
                            "id" => $value["value_index"], "label" => $value["label"],
                           'products' => ($childProduct->isSalable()) ? [ $childProduct->getId() ] : []
                        );
                        /** get product child id  */
                    }
                }
        ...
        $customerToken, $currency, $sku="")
    {
        ...
        try {
            ...
            /** Amasty labels  */
             $this->getAmastyLabelByProduct($productId,$_returnArray);
    ...
    protected function getProductBasicDeatils($productId,&$returnArray,$width)
    {
        ...
        /** product image url change */
        $returnArray["thumbNail"] = $this->helperCatalog->getImageUrlProduct($this->product);
    ...
    protected function getAdditionalInformation()
    {
     ...
     public function getAmastyLabelByProduct($productId, &$_returnArray){

        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $resource   = $objectManager->get('Magento\Framework\App\ResourceConnection');
        $connection = $resource->getConnection();
        $sql        = "SELECT * FROM `amasty_label_entity` WHERE `status`= 1 ORDER BY `priority` DESC" ;
        $result     = $connection->fetchAll($sql);
        $amastyLabels = [];

        $arr = $finalArr       =  [];
        /* Custom Work for Multiple labels get */
        foreach ($result as $key => $value) {
            if($productId != 0) {
                $sql        = "SELECT * FROM `amasty_label_index` WHERE `label_id`= ".$value["label_id"]." AND `product_id` = ".$productId."" ;
                $result2    = $connection->fetchRow($sql);
                if(!empty($result2)) {
                    $arr        = $value;
                    $sql        = "SELECT * FROM `amasty_label_catalog_parts` WHERE `label_id`= ".$result2["label_id"]." AND `type` = 2" ;
                    $result1    = $connection->fetchAll($sql);

                    if ($result1[0]['type'] == 2){
                        $result1[0]['type'] = 'product';
                    }
                    if(!empty($result1[0]['image'])) {
                        $mediaUrl = $this->storeInterface->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA);
                        $mediaUrl = $mediaUrl.'/amasty/amlabel/';
                        $result1[0]['image'] = $mediaUrl.$result1[0]['image'];
                    } else{
                        $result1[0]['image'] = null;
                    }

                    $arr['data'] = $result1[0];
                    $amastyLabels[] = $arr;
                    $finalArr = $amastyLabels;
                }
            }
        }
        $_returnArray['amastylabel']  = $finalArr;
    }
