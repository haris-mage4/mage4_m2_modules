...
class CategoryManagement
{
        ...
        1, $websiteId = 1, $quoteId = 0, $storeId = 1,$filterData = "")
    {
        ...
        if(is_array($sortData) && isset($sortData["code"])){
            $sortBy = $sortData["code"];
            $sorting = "DESC";
            if(isset($sortData["sort"])){
                $sorting = strtoupper($sortData["sort"]); 
            }
            $isFlatEnabled = $collection->isEnabledFlat();
        if ($isFlatEnabled) {
                    $collection->setOrder($sortBy, $sorting);
                } else {
                    $collection->addAttributeToSort($sortBy, $sorting);
                }    
            }
        // sorting
